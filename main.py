...
# Optional Supabase – if not available, fall back to local files
try:
...
HEART_API_URL = os.environ.get("HEART_API_URL")  # optional external heart API
LOCAL_STATE_FILE = "sovereign_state.json"
...
if SUPABASE_AVAILABLE and SUPABASE_URL and SUPABASE_KEY:
    ...
    db = None
    logger.warning("Supabase not available. Using local JSON file for persistence.")
...
def get_memory():
    ...
    _write_local_state(default_state)
    return default_state

def save_memory(state):
    if db:
        try:
            db.table("sovereign_state").update(
                {"state_data": state, "updated_at": datetime.utcnow().isoformat()}
            ).eq("id", 1).execute()
        except Exception as e:
...
async def swarm_consensus(prompt: str):
    ...
    if not valid_results:
        return "Swarm Overload: All APIs offline."
...
async def reconcile_memory():
    """Update heart_successes from external heart API if configured."""
    while True:
        try:
            state = get_memory()
            if HEART_API_URL:
                async with httpx.AsyncClient() as client:
                    ...
                    if resp.status_code == 200:
                        data = resp.json()
                        state["heart_successes"] = data.get("successes", state.get("heart_successes", 0))
                        state["master_successes"] = 439434 + state["lung_successes"] + state["heart_successes"]
                        save_memory(state)
        except Exception as e:
            logger.error(f"Heart reconcile error: {e}")
        await asyncio.sleep(10)

async def lung_evolution_cycle():
    """Main evolution loop: generate, audit, and persist improvements."""
    ...
    while True:
        try:
            ...
            # --- GENERATE HYPOTHESIS (auto‑heal if Gemini fails) ---
            ...
            if not hypothesis:
                # No hypothesis, wait and try again
            ...
            if audit_res:
                try:
                    audit_score = int(''.join(filter(str.isdigit, audit_res)))
                except ValueError:
                    pass
            ...
            if len(state["lung_logs"]) > 15:
                state["lung_logs"].pop(0)
            save_memory(state)
...
async def multi_ai_chat(req: MultiAIChat):
    ...
    if not response:
        response = "Neural Link Degraded. Please try again."
